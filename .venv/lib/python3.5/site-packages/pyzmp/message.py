...
def force_protobuf_implementation():
    ...
    ServiceRequest._parse = lambda s, m: [s, s.ParseFromString(m)][
        0]  # returns itself after modification during parsing
    # Factory Function for oneline parsing creation
    ServiceRequest_dictparse = lambda m: ServiceRequest(
        **{f.name: v for (f, v) in ServiceRequest()._parse(m).ListFields()})
    ...
    ServiceResponse._parse = lambda s, m: [s, s.ParseFromString(m)][
        0]  # returns itself after modification during parsing
    # Factory Function for oneline parsing creation
    ServiceResponse_dictparse = lambda m: ServiceResponse(
        **{f.name: v for (f, v) in ServiceResponse()._parse(m).ListFields()})
    ...
    ServiceException._parse = lambda s, m: [s, s.ParseFromString(m)][
        0]  # returns itself after modification during parsing
    # Factory Function for oneline parsing creation
    ServiceException_dictparse = lambda m: ServiceException(
        **{f.name: v for (f, v) in ServiceException()._parse(m).ListFields()})
...
def force_namedtuple_implementation():
    ...
    ServiceRequest.__new__.__defaults__ = (None,) * len(
        ServiceRequest._fields)  # making all fields optional ( like in protobuf protocol )
    ...
    ServiceResponse.__new__.__defaults__ = (None,) * len(
        ServiceResponse._fields)  # making all fields optional ( like in protobuf protocol )
    ...
    ServiceException.__new__.__defaults__ = (None,) * len(
        ServiceException._fields)  # making all fields optional ( like in protobuf protocol )
